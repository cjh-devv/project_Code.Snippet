...
router.post('/update', (req, res, next) => {
    ...
    try {
        ...
        if (isDeleteImage === 'true') {
            ...
            if (oldProfileImage && oldProfileImage.includes('uploads/')) {
                const oldFilePath = oldProfileImage.split(host)[1];
        ...
        if (req.file) {
            ...
            await connection.execute(
                `UPDATE USERS SET NICKNAME = :nickname, PROFILE_IMAGE = :profileImage WHERE USER_ID = :userId`,
            ...
            if (oldProfileImage && oldProfileImage.includes('uploads/')) {
                const oldFilePath = oldProfileImage.split(host)[1]; // 'uploads/파일명' 추출
