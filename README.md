...
| 구분 | 주요 기능 |
|------|----------|
| 🔐 회원 인증 | JWT 기반 회원가입·로그인, Access Token 인증, 로그인 상태 유지 |
| 📝 게시글 | 개발 노트 및 코드 스니펫 CRUD, 코드 블록 등록, 조회수 집계 |
| 💻 코드 스니펫 | 코드 블록 표시, 원클릭 코드 복사, 트러블슈팅 및 학습 내용 기록 |
| 🏷️ 태그·검색 | 태그 등록·수정, 게시글 및 태그 기반 검색 |
| ❤️ 소셜 | 좋아요, 북마크, 댓글 작성·삭제 |
| 👤 마이페이지 | 작성 게시글, 북마크 게시글, 프로필 및 자기소개 관리 |
| 📱 사용자 경험 | 피드·상세 페이지 구성, 코드 복사 편의 기능 |
